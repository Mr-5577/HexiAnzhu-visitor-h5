  ...
  <view class="visitor-register-page">
    ...
    <view class="visitor-type-card">
      ...
      <view class="type-value">{{
        visitorType === "delivery" ? "外卖人员" : "普通访客"
      }}</view>
    </view>
    ...
    <view class="register-info" v-if="hasHistory">
      ...
      <view class="visit-user">
        ...
        <text class="user-text">手机号：{{ form.phone }}</text>
      </view>
    </view>
      ...
      <view class="form-section">
        ...
        <view class="form-item" v-if="visitorType === 'normal'">
          ...
          >
            <view
              ...
              :class="{ disabled: !form.communityId }"
            >
        ...
        <view class="form-item" v-if="visitorType === 'normal'">
          ...
          <input
            ...
            placeholder="请填写业主手机号"
            type="number"
            maxlength="11"
            @blur="validateOwnerPhone"
          />
          <view v-if="ownerPhoneError" class="error-text">{{
            ownerPhoneError
          }}</view>
        </view>
      </view>
...
export default {
  ...
  computed: {
    // 是否可以提交
    canSubmit() {
      const { visitorName, idCard, phone } = this.form;
      const baseValid =
        visitorName && idCard && phone && !this.idCardError && !this.phoneError;
  ...
  onLoad(options) {
    ...
    // 3. 没有历史记录，获取openId登录，加载小区列表手动录入
    if (!hasValidHistory) {
      this.getOpenId();
  ...
  methods: {
    ...
    async getOpenId() {
      ...
      this.$api.getUserOpenid({ code: codeRes[1].code }, (res) => {
